...
async function fetchTefasFundData(fundCode, days = 30) {
    ...
    try {
        const workerUrl = `${IS_YATIRIM_WORKER_URL}?fon=${encodeURIComponent(fCode)}&days=${days}&_t=${Date.now()}`;
...
function generateFallbackFundTrajectory(fundCode, days = 30) {
    const knownFunds = {
        "TI1": { name: "İŞ PORTFÖY PARA PİYASASI (TL) FONU", price: 1719.161631, investors: 234157, shares: 120263620, dailyYield: 0.002995 },
        ...
        "MAC": { name: "MARMARA CAPITAL PORTFÖY HİSSE SENEDİ (TL) FONU (HİSSE SENEDİ YOĞUN FON)", price: 0.708156, investors: 34565, shares: 5570885818, dailyYield: -0.0194 },
        "IIH": { name: "İSTANBUL PORTFÖY ÜÇÜNCÜ HİSSE SENEDİ FONU", price: 38.65, investors: 78500, shares: 65100000, dailyYield: 0.0016 },
        "TCD": { name: "TACİRLER PORTFÖY DEĞİŞKEN FON", price: 47.569393, investors: 11421, shares: 29173638, dailyYield: -0.0015 },
...
async function fetchTefasFundAllocation(fundCode, days = 30) {
    ...
    try {
        const workerUrl = `${IS_YATIRIM_WORKER_URL}?fon=${encodeURIComponent(fCode)}&days=${days}&alloc=1&_t=${Date.now()}`;
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 9000);
        const res = await fetch(workerUrl, { signal: controller.signal });
        clearTimeout(timeoutId);
...
async function loadAndRenderFundCategories(forceRefresh = false) {
    ...
    if (fundLeadersDataCache && fundLeadersDataCache.categories) {
        ...
        leaderCats.forEach(f => {
            ...
            const aum = parseFloat(f.aum) || (existing ? existing.aum : 0);
            ...
            const investors = parseInt(f.investors) || (existing ? existing.investors : 0);
            let change = (typeof f.change === 'number' && !isNaN(f.change)) ? f.change : (existing?.change !== undefined ? existing.change : 0);
            if (!change && f.prevPrice > 0 && price > 0) {
                change = Number((((price - f.prevPrice) / f.prevPrice) * 100).toFixed(2));
            }
...
async function loadAndRenderFundLeaders(forceRefresh = false) {
    ...
    const cacheKey = "tefas_leaders_cache_v6";
        ...
        try {
            ...
            if (cachedStr) {
                ...
                if (parsed && parsed.timestamp && (Date.now() - parsed.timestamp) < 900000 && parsed.categories && (parsed.categories.topInvestorInflow?.length || 0) > 3) {
                    if (!parsed.date || parsed.date >= "2026-09-21") {
    ...
    try {
        ...
        fundLeadersDataCache = {
            timestamp: Date.now(),
            date: leadersResult.date || "2026-09-21",
    ...
    for (const chunk of chunks) {
        ...
        for (let j = 0; j < chunk.length; j++) {
            ...
            if (data && data.length >= 2) {
                if (data.some(r => r.isFallback)) continue;
    ...
    "topInvestorInflow": [
      {
        ...
        "date": "2026-09-21",
        "price": 1719.161631,
        "prevPrice": 1714.028004,
        "change": 0.2995,
        "aum": 206752602215.76,
        "investors": 234157,
        "prevInvestors": 233245,
        "deltaInvestors": 912,
        "deltaShares": 1447932,
        "cashFlow": 2489229138.697092,
        "perPerson": 2729418
      },
      {
        "code": "ZBJ",
        ...
        "prevInvestors": 21523,
        "deltaInvestors": 418,
        "deltaShares": -320392632,
        "cashFlow": -1711641247.356768,
        "perPerson": 4094836
      },
      {
        "code": "PJL",
        "name": "PHİLLİP PORTFÖY PARA PİYASASI FONU",
        "date": "2026-09-21",
        "price": 5.359846,
        "prevPrice": 5.344395,
        "change": 0.2891,
      ...
      },
      {
        ...
        "prevInvestors": 1461,
        "deltaInvestors": 298,
        "deltaShares": 2863636622,
        "cashFlow": 3585407641.6652336,
        "perPerson": 12031569
      },
      {
        "code": "IOO",
        ...
        "prevInvestors": 9019,
        "deltaInvestors": 290,
        "deltaShares": 300043278,
        "cashFlow": 1425334289.0662618,
        "perPerson": 4914946
      },
      {
        "code": "ZPO",
        "name": "ZİRAAT PORTFÖY PARA PİYASASI KATILIM FONU",
        "date": "2026-09-21",
        "price": 1.073697,
        "prevPrice": 1.070516,
        "change": 0.2971,
        ...
        "prevInvestors": 2538,
        "deltaInvestors": 226,
        "deltaShares": 100224474,
        "cashFlow": 107610717.06037799,
        "perPerson": 476154
      },
      {
        "code": "TSI",
        ...
        "prevInvestors": 76666,
        "deltaInvestors": 154,
        "deltaShares": -184906767,
        "cashFlow": -39445420.977042,
        "perPerson": 256139
      },
      {
        "code": "AIS",
        ...
        "prevInvestors": 24066,
        "deltaInvestors": 147,
        "deltaShares": 203418170,
        "cashFlow": 22556227.19862,
        "perPerson": 153444
      },
      {
        "code": "THF",
        ...
        "aum": 40452359335.39,
        "investors": 214405,
        "prevInvestors": 214299,
        "deltaInvestors": 106,
        "deltaShares": 3304202,
        "cashFlow": 6295152.433592,
        "perPerson": 59388
      },
      {
        "code": "GPN",
        "name": "GARANTİ PORTFÖY PARA PİYASASI KATILIM (TL) FONU",
        "date": "2026-09-21",
        "price": 1.435456,
        "prevPrice": 1.430991,
        "change": 0.312,
        ...
        "prevInvestors": 5100,
        "deltaInvestors": 93,
        "deltaShares": -286036301,
        "cashFlow": -410592524.48825604,
        "perPerson": 4414973
      },
      {
        "code": "BGP",
        ...
        "prevInvestors": 28757,
        "deltaInvestors": 81,
        "deltaShares": 111940004,
        "cashFlow": 754913424.315644,
        "perPerson": 9319919
      },
      {
        "code": "KPI",
        "name": "İŞ PORTFÖY PARA PİYASASI KATILIM (TL) FONU",
        "date": "2026-09-21",
        "price": 1.518108,
        "prevPrice": 1.513707,
        "change": 0.2907,
        ...
        "prevInvestors": 7099,
        "deltaInvestors": 81,
        "deltaShares": -77660626,
        "cashFlow": -117897217.615608,
        "perPerson": 1455521
      },
      {
        "code": "ZPK",
      ...
      },
      {
        ...
        "prevInvestors": 1024,
        "deltaInvestors": 68,
        "deltaShares": 1564039682,
        "cashFlow": 2492505250.5447063,
        "perPerson": 36654489
      },
      {
        "code": "ILH",
        ...
        "prevInvestors": 11367,
        "deltaInvestors": 67,
        "deltaShares": -2651003564,
        "cashFlow": -10951248004.819849,
        "perPerson": 163451463
      }
    ],
    "topInvestorOutflow": [
      {
        "code": "KHA",
        "name": "PARDUS PORTFÖY İKİNCİ HİSSE SENEDİ (TL) FONU (HİSSE SENEDİ YOĞUN FON)",
        "date": "2026-09-21",
        "price": 4.180371,
        "prevPrice": 4.309448,
        "change": -2.9952,
        ...
        "prevInvestors": 29972,
        "deltaInvestors": -4835,
        "deltaShares": -33047263,
        "cashFlow": -138149819.874573,
        "perPerson": 28573
      },
      {
        "code": "FSU",
        ...
        "prevInvestors": 13025,
        "deltaInvestors": -3096,
        "deltaShares": 64169150,
        "cashFlow": 89158798.76110001,
        "perPerson": 28798
      },
      {
        "code": "DLY",
        "name": "DENİZ PORTFÖY PARA PİYASASI (TL) FONU",
        "date": "2026-09-21",
        "price": 6.261427,
        "prevPrice": 6.243088,
        "change": 0.2937,
        "aum": 60452188585.29,
        "investors": 307713,
        "prevInvestors": 308893,
        "deltaInvestors": -1180,
        "deltaShares": -245118835,
        "cashFlow": -1534793691.677545,
        "perPerson": 1300673
      },
      {
        "code": "HLL",
        ...
        "aum": 51107859069.56,
        "investors": 1031063,
        "prevInvestors": 1031935,
        "deltaInvestors": -872,
        "deltaShares": -2273436795,
        "cashFlow": -896002362.77181,
        "perPerson": 1027526
      },
      {
        "code": "TIV",
        "name": "İŞ PORTFÖY KISA VADELİ BORÇLANMA ARAÇLARI (TL) FONU",
        "date": "2026-09-21",
        "price": 54.609603,
        "prevPrice": 54.461307,
        "change": 0.2723,
        "aum": 36490409791.53,
        "investors": 126196,
        "prevInvestors": 126816,
        "deltaInvestors": -620,
        "deltaShares": -22511194,
        "cashFlow": -1229327367.395982,
        "perPerson": 1982786
      },
      {
        "code": "ICH",
        "name": "PARDUS PORTFÖY İKİNCİ DEĞİŞKEN FON",
        "date": "2026-09-21",
        "price": 2.388901,
        "prevPrice": 2.419516,
        "change": -1.2653,
      ...
      },
      {
        ...
        "aum": 26375871857.35,
        "investors": 225524,
        "prevInvestors": 226107,
        "deltaInvestors": -583,
        "deltaShares": -44626959,
        "cashFlow": -27985298.227146003,
        "perPerson": 48002
      },
      {
        "code": "TKM",
        ...
        "aum": 73434545042.88,
        "investors": 502008,
        "prevInvestors": 502587,
        "deltaInvestors": -579,
        "deltaShares": -10794607222,
        "cashFlow": -2395744332.243458,
        "perPerson": 4137728
      },
      {
        "code": "ALE",
        ...
        "aum": 78140660554.55,
        "investors": 139789,
        "prevInvestors": 140293,
        "deltaInvestors": -504,
        "deltaShares": 81483272,
        "cashFlow": 1135137432.469872,
        "perPerson": 2252257
      },
      {
        "code": "TTE",
        "name": "İŞ PORTFÖY BIST TEKNOLOJİ AĞIRLIK SINIRLAMALI ENDEKSİ HİSSE SENEDİ (TL) FONU (HİSSE SENEDİ YOĞUN FON)",
        "date": "2026-09-21",
        "price": 1.323616,
        "prevPrice": 1.379455,
        "change": -4.0479,
        ...
        "prevInvestors": 44388,
        "deltaInvestors": -460,
        "deltaShares": -16324785,
        "cashFlow": -21607746.62256,
        "perPerson": 46973
      },
      {
        "code": "AFT",
        ...
        "aum": 18918597153.02,
        "investors": 135754,
        "prevInvestors": 136203,
        "deltaInvestors": -449,
        "deltaShares": -84327203,
        "cashFlow": -84769077.54371999,
        "perPerson": 188795
      },
      {
        "code": "PPN",
        "name": "NUROL PORTFÖY PARA PİYASASI (TL) FONU",
        "date": "2026-09-21",
        "price": 5.958166,
        "prevPrice": 5.939979,
        "change": 0.3062,
        ...
        "prevInvestors": 12092,
        "deltaInvestors": -443,
        "deltaShares": -72571562,
        "cashFlow": -432393413.27529204,
        "perPerson": 976057
      },
      {
        "code": "LTL",
        "name": "AHLATCI PORTFÖY HİSSE SENEDİ (TL) FONU (HİSSE SENEDİ YOĞUN FON)",
        "date": "2026-09-21",
        "price": 1.160209,
        "prevPrice": 1.162871,
        "change": -0.2289,
        ...
        "prevInvestors": 4350,
        "deltaInvestors": -420,
        "deltaShares": -41159106,
        "cashFlow": -47753165.213154,
        "perPerson": 113698
      },
      {
        "code": "IJC",
      ...
      },
      {
        ...
        "prevInvestors": 1882,
        "deltaInvestors": -378,
        "deltaShares": -42302382,
        "cashFlow": -44305357.485318005,
        "perPerson": 117210
      }
    ],
    "topCashInflow": [
      {
        "code": "YPT",
        "name": "YAPI KREDİ PORTFÖY PARA PİYASASI SERBEST (TL) FON",
        "date": "2026-09-21",
        "price": 4.272823,
        "prevPrice": 4.260194,
        "change": 0.2964,
        ...
        "prevInvestors": 19305,
        "deltaInvestors": -13,
        "deltaShares": 2106589620,
        "cashFlow": 9001084579.897259,
        "perPerson": 692391122
      },
      {
        "code": "GTL",
        ...
        "aum": 176620221881.43,
        "investors": 349927,
        "prevInvestors": 350115,
        "deltaInvestors": -188,
        "deltaShares": 45974134374,
        "cashFlow": 6383232713.023656,
        "perPerson": 33953365
      },
      {
        "code": "KHP",
      ...
      },
      {
        ...
        "prevInvestors": 38931,
        "deltaInvestors": 36,
        "deltaShares": 178931014,
        "cashFlow": 1927659421.093786,
        "perPerson": 53546095
      },
      {
        "code": "NBH",
        "name": "NEO PORTFÖY ALGORİTMİK STRATEJİLER SERBEST FON",
        "date": "2026-09-21",
        "price": 7.26881,
        "prevPrice": 7.262002,
        "change": 0.0937,
        ...
        "prevInvestors": 201,
        "deltaInvestors": -3,
        "deltaShares": 260097904,
        "cashFlow": 1890602245.57424,
        "perPerson": 630200749
      },
      {
        "code": "TZL",
        ...
        "aum": 96786337996.84,
        "investors": 185402,
        "prevInvestors": 185618,
        "deltaInvestors": -216,
        "deltaShares": 12913539100,
        "cashFlow": 1839611126.0296,
        "perPerson": 8516718
      },
      {
        "code": "DIP",
        ...
        "prevInvestors": 2020,
        "deltaInvestors": 11,
        "deltaShares": 1090081264,
        "cashFlow": 1777711065.818784,
        "perPerson": 161610097
      },
      {
        "code": "NUG",
        "name": "NUROL PORTFÖY ALTINCI SERBEST (DÖVİZ) FON",
        "date": "2026-09-21",
        "price": 50.096661,
        "prevPrice": 50.041343,
        "change": 0.1105,
        ...
        "prevInvestors": 22,
        "deltaInvestors": 0,
        "deltaShares": 30123523,
        "cashFlow": 1509087919.8567028,
        "perPerson": 0
      },
      {
        "code": "IOO",
      ...
      {
        "code": "NKL",
        "name": "AKTİF PORTFÖY KISA VADELİ SERBEST (TL) FON",
        "date": "2026-09-21",
        "price": 3.660052,
        "prevPrice": 3.64934,
        "change": 0.2935,
        ...
        "prevInvestors": 2106,
        "deltaInvestors": 65,
        "deltaShares": 319097108,
        "cashFlow": 1167912008.329616,
        "perPerson": 17967877
      },
      {
        "code": "ALE",
      ...
      {
        "code": "VKI",
        "name": "ALBARAKA PORTFÖY VAKIF KATILIM İKİNCİ KISA VADELİ KATILIM SERBEST (TL) FON",
        "date": "2026-09-21",
        "price": 3.202564,
        "prevPrice": 3.193514,
        "change": 0.2834,
        ...
        "prevInvestors": 345,
        "deltaInvestors": -4,
        "deltaShares": 341028248,
        "cashFlow": 1092164790.027872,
        "perPerson": 273041198
      },
      {
        "code": "VFK",
        "name": "ZİRAAT PORTFÖY İKİNCİ KISA VADELİ KİRA SERTİFİKALARI KATILIM (TL) FONU",
        "date": "2026-09-21",
        "price": 5.635866,
        "prevPrice": 5.620175,
        "change": 0.2792,
        ...
        "prevInvestors": 5829,
        "deltaInvestors": 4,
        "deltaShares": 179561844,
        "cashFlow": 1011986491.496904,
        "perPerson": 252996623
      }
    ],
    "topCashOutflow": [
      {
        "code": "GSO",
        "name": "GARANTİ PORTFÖY ÖPY SERBEST ÖZEL FON",
        "date": "2026-09-21",
        "price": 15.794352,
        "prevPrice": 15.743912,
        "change": 0.3204,
        ...
        "prevInvestors": 8,
        "deltaInvestors": -1,
        "deltaShares": -2889448622,
        "cashFlow": -45636968621.78294,
        "perPerson": 45636968622
      },
      {
        "code": "GJH",
        "name": "GARANTİ PORTFÖY PARA PİYASASI SERBEST (TL) FON",
        "date": "2026-09-21",
        "price": 4.057512,
        "prevPrice": 4.045783,
        "change": 0.2899,
        ...
        "prevInvestors": 23339,
        "deltaInvestors": -15,
        "deltaShares": -2897692702,
        "cashFlow": -11757422910.677423,
        "perPerson": 783828194
      },
      {
        "code": "ILH",
      ...
      {
        "code": "URM",
        "name": "AK PORTFÖY ÖPY ALTINCI SERBEST (DÖVİZ) ÖZEL FON",
        "date": "2026-09-21",
        "price": 49.92094,
        "prevPrice": 49.874947,
        "change": 0.0922,
        ...
        "prevInvestors": 5,
        "deltaInvestors": 0,
        "deltaShares": -97418223,
        "cashFlow": -4863209265.28962,
        "perPerson": 0
      },
      {
        "code": "BKO",
        "name": "AZİMUT PORTFÖY BİRİNCİ KATILIM SERBEST ÖZEL FON",
        "date": "2026-09-21",
        "price": 1.31922,
        "prevPrice": 1.315344,
        "change": 0.2947,
      ...
      },
      {
        ...
        "prevInvestors": 2236,
        "deltaInvestors": -63,
        "deltaShares": -701263452,
        "cashFlow": -2763943640.653404,
        "perPerson": 43872121
      },
      {
        "code": "TKM",
      ...
      {
        "code": "DOV",
        "name": "DENİZ PORTFÖY ONÜÇÜNCÜ SERBEST (DÖVİZ-AVRO) FON",
        "date": "2026-09-21",
        "price": 61.081021,
        "prevPrice": 61.019055,
        "change": 0.1016,
        ...
        "prevInvestors": 17140,
        "deltaInvestors": -84,
        "deltaShares": -37322065,
        "cashFlow": -2279669836.028365,
        "perPerson": 27138927
      },
      {
        "code": "DAS",
        ...
        "prevInvestors": 25168,
        "deltaInvestors": -115,
        "deltaShares": -37593612,
        "cashFlow": -2113171178.300532,
        "perPerson": 18375402
      },
      {
        "code": "FI5",
        "name": "QNB PORTFÖY PARA PİYASASI (TL) FONU",
        "date": "2026-09-21",
        "price": 765.515279,
        "prevPrice": 763.259439,
        "change": 0.2956,
        "aum": 118670822291.07,
        "investors": 52901,
        "prevInvestors": 53165,
        "deltaInvestors": -264,
        "deltaShares": -2613928,
        "cashFlow": -2001001822.2059119,
        "perPerson": 7579552
      },
      {
        "code": "ZUE",
        "name": "ZİRAAT PORTFÖY TS SERBEST ÖZEL FON",
        "date": "2026-09-21",
        "price": 5.23042,
        "prevPrice": 5.2337,
        "change": -0.0627,
        ...
        "prevInvestors": 1,
        "deltaInvestors": 0,
        "deltaShares": -351260114,
        "cashFlow": -1837237925.4678798,
        "perPerson": 0
      },
      {
        "code": "KAN",
        "name": "KUVEYT TÜRK PORTFÖY ALTIN KATILIM SERBEST FON",
        "date": "2026-09-21",
        "price": 6977.292734,
        "prevPrice": 6882.902762,
        "change": 1.3714,
        "aum": 3664020619.69,
        "investors": 27,
        "prevInvestors": 35,
      ...
      },
      {
        ...
        "prevInvestors": 30562,
        "deltaInvestors": -132,
        "deltaShares": -29499167,
        "cashFlow": -1714938321.4250479,
        "perPerson": 12991957
      },
      {
        "code": "ZBJ",
...
function openPinPreviewMode() {
    ...
    if (previewBanner) previewBanner.style.display = "inline-flex";
...
function exitPinPreview() {
    ...
    if (overlay) {
        ...
        overlay.style.transition = "all 0.28s cubic-bezier(0.16, 1, 0.3, 1)";
        setTimeout(() => {
            overlay.style.display = "none";
            overlay.style.opacity = "1";
            overlay.style.transform = "scale(1)";
...
function pressPin(num) {
    if (enteredPin.length < 4) {
        ...
        if (badge) {
            ...
            setTimeout(() => badge.classList.remove("pulse-glow"), 180);
        }
...
function verifyPin() {
    ...
    if (isMatch) {
        ...
        setTimeout(() => {
            if (dotsContainer) dotsContainer.classList.remove("shake-error");
...
function buildSlideReportDataset() {
    ...
    const leaders = fundLeadersDataCache?.categories || {};

    // Helper to format a leader fund cleanly with genuine numbers
    const formatLeaderFund = (f) => {
        if (!f || !f.code) return null;
        const code = String(f.code).toUpperCase().trim();
        const cat = f.category || (typeof detectFundCategoryKey === 'function' ? detectFundCategoryKey(f.name, code) : "DİĞER");
        const price = parseFloat(f.price) || 0;
        const prevPrice = parseFloat(f.prevPrice) || 0;
        let change = (typeof f.change === 'number' && !isNaN(f.change)) ? f.change : null;
        if (change === null && prevPrice > 0 && price > 0) {
            change = Number((((price - prevPrice) / prevPrice) * 100).toFixed(2));
        }
        if (change === null) change = 0;

        const aum = parseFloat(f.aum) || 0;
        const cashFlow = parseFloat(f.cashFlow) || 0;
        const deltaInvestors = parseInt(f.deltaInvestors) || 0;
        const investors = parseInt(f.investors) || 0;
        let perPerson = parseFloat(f.perPerson) || 0;
        if (!perPerson && deltaInvestors !== 0 && cashFlow !== 0) {
            perPerson = Math.round(Math.abs(cashFlow / deltaInvestors));
        }

        return {
            code,
            name: f.name || `${code} FONU`,
            category: cat,
            price,
            prevPrice,
            change,
    ...
    // Filter for valid public funds with real price and AUM
    const filterValid = list => (list || [])
        .map(formatLeaderFund)
        .filter(f => f && f.price > 0 && f.aum > 0);

    let topCashInflow = filterValid(leaders.topCashInflow).slice(0, 3);
    let topCashOutflow = filterValid(leaders.topCashOutflow).slice(0, 3);
    let topInvestorInflow = filterValid(leaders.topInvestorInflow).slice(0, 3);
    let topInvestorOutflow = filterValid(leaders.topInvestorOutflow).slice(0, 3);
    ...
    const stats = typeof calculateCategoryMetrics === 'function' 
        ? calculateCategoryMetrics(allFunds) 
        : { categories: {}, marketOverview: { totalFunds: allFunds.length, totalAUM: 0, dailyCashFlow: 0, dailyInvestors: 0 } };

    // Category leaders (Inflow & Outflow)
        ...
        const step = () => {
            ...
            if (liveFundScannerShouldSkip) {
                ...
                if (liveScannerSpeedMode === 'slow') {
                    ...
                    currentBatchSize = 1; // Normal: 2,041 * 29ms = 59.2s (~60 sn)
                    currentDelay = 29;
                }
                ...
                for (let i = processed; i < end; i++) {
                    ...
                    if (liveScannerSpeedMode === 'slow') {
                        ...
                        // In normal mode (60s), stream every single fund to terminal
                        shouldAdd = true;
                    }
...
function openFundSlideReportModal() {
    ...
    try {
        ["tefas_leaders_cache_v1", "tefas_leaders_cache_v2", "tefas_leaders_cache_v3", "tefas_leaders_cache_v4", "tefas_leaders_cache_v5"].forEach(k => localStorage.removeItem(k));
    } catch (e) {}

    // Hydrate from v5 cache if in-memory is empty
    if (!fundLeadersDataCache) {
        try {
            const cachedStr = localStorage.getItem("tefas_leaders_cache_v6");
            if (cachedStr) {
                const parsed = JSON.parse(cachedStr);
                if (parsed && parsed.categories && (!parsed.date || parsed.date >= "2026-09-21")) {
                    fundLeadersDataCache = parsed;
                }
            }
        } catch (e) {}
...
async function openSlideFundDetail(fundCode, originSlideIndex = null) {
    ...
    // 3. Resolve Metrics with authoritative historyData override
    const latestHist = historyData.length > 0 ? historyData[historyData.length - 1] : null;
    const prevHist = historyData.length >= 2 ? historyData[historyData.length - 2] : null;
    const firstHist = historyData.length > 0 ? historyData[0] : null;

    let rawPrice = fundMeta?.price || 1.0;
    let retVal = (typeof fundMeta?.change === 'number' && !isNaN(fundMeta.change)) ? fundMeta.change : 0;
    let aumVal = fundMeta?.aum || 0;
    let cashVal = fundMeta?.cashFlow || 0;
    let deltaInvVal = fundMeta?.deltaInvestors !== undefined ? fundMeta.deltaInvestors : 0;
    let totalInvestors = fundMeta?.investors || 0;
    let perPersonVal = fundMeta?.perPerson || 0;

    // Direct live override from TEFAS historyData when available
    if (latestHist) {
        if (latestHist.fiyat) rawPrice = parseFloat(latestHist.fiyat) || rawPrice;
        if (latestHist.portfoyBuyukluk) aumVal = parseFloat(latestHist.portfoyBuyukluk) || aumVal;
        if (latestHist.kisiSayisi !== undefined && latestHist.kisiSayisi !== null) {
            totalInvestors = parseInt(latestHist.kisiSayisi) || totalInvestors;
        }
    }
    if (latestHist && prevHist) {
        const pCurr = parseFloat(latestHist.fiyat) || 0;
        const pPrev = parseFloat(prevHist.fiyat) || 0;
        if (pPrev > 0 && pCurr > 0) {
            retVal = Number((((pCurr - pPrev) / pPrev) * 100).toFixed(2));
        }
        const invCurr = parseInt(latestHist.kisiSayisi) || 0;
        const invPrev = parseInt(prevHist.kisiSayisi) || 0;
        if (invCurr > 0 && invPrev > 0) {
            deltaInvVal = invCurr - invPrev;
        }
        const sharesCurr = parseFloat(latestHist.tedPaySayisi) || 0;
        const sharesPrev = parseFloat(prevHist.tedPaySayisi) || 0;
        if (sharesCurr > 0 && sharesPrev > 0 && pCurr > 0) {
            cashVal = (sharesCurr - sharesPrev) * pCurr;
        }
        if (deltaInvVal !== 0 && cashVal !== 0) {
            perPersonVal = Math.round(Math.abs(cashVal / deltaInvVal));
        }
    }
    if (retVal === undefined || retVal === null || isNaN(retVal)) {
        retVal = 0;
    }

    // Keep fundMeta updated in memory with fresh live numbers
    if (fundMeta) {
        fundMeta.price = rawPrice;
        fundMeta.change = retVal;
        fundMeta.aum = aumVal;
        fundMeta.cashFlow = cashVal;
        fundMeta.deltaInvestors = deltaInvVal;
        fundMeta.investors = totalInvestors;
        fundMeta.perPerson = perPersonVal;
    }
    ...
    const deltaInvStr = `${deltaInvVal >= 0 ? '+' : ''}${Number(deltaInvVal).toLocaleString('tr-TR')} Kişi`;
